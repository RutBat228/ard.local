...
function prim_zp($fio, $month_name, $year) {
    ...
    // Массив месяцев для преобразования названия в номер
    $months = array(
    ...
    // Запрос для получения всех данных: дежурства, аванс, монтаж, регион и бонус
    $sql = "SELECT 
        COALESCE(uf.dejurstva, 0) as dejurstva, -- Если дежурств нет (NULL), берем 0
        u.id as user_id,
        COALESCE(uf.advance, 0) as advance, -- Если аванса нет (NULL), берем 0
        uf.official_employment,
        u.region,
        COALESCE(c.monthly_bonus, 10.00) as monthly_bonus, -- Бонус по умолчанию 10%, если не указан
        COALESCE(
            (SELECT SUM(kajdomu) 
             FROM montaj 
        ...
        ) as montaj_sum -- Сумма монтажей, если нет — 0
        FROM user u
    ...
    $stmt = $connect->prepare($sql);
    $month_num_int = intval($month_num); // Преобразуем номер месяца в integer
    $year_int = intval($year);           // Преобразуем год в integer
    ...
    // Если пользователь не найден, выводим сообщение и выходим
    if (!$data) {
        echo "Пользователь не найден";
        return;
    }
    
    // Расчет суммы за дежурства (1300 рублей за единицу дежурства)
    $dejurstva_sum = $data['dejurstva'] * 1300;
    $montaj_sum = $data['montaj_sum'];              // Сумма за монтажи
    $advance = $data['advance'];                    // Аванс
    $official_employment = $data['official_employment']; // Официальное трудоустройство
    $monthly_bonus = $data['monthly_bonus'];        // Процент бонуса из config
    
    // Расчет бонуса (в процентах от суммы монтажей и дежурств)
    $bonus = ($montaj_sum + $dejurstva_sum) * ($monthly_bonus / 100);
    
    // Расчет итоговых сумм в зависимости от официального трудоустройства
    if ($official_employment === 'Да' || $official_employment === '1') {
        $card_sum = 24000;                          // Фиксированная часть на карту
        $cash_sum = $montaj_sum + $dejurstva_sum + $bonus - $advance; // Наличные
        $total_sum = $card_sum + $cash_sum + $advance; // Общая сумма с учетом аванса
    } else {
        $total_sum = $montaj_sum + 24000 + $dejurstva_sum + $bonus; // Общая сумма
        $cash_sum = $total_sum - $advance;          // Наличные после вычета аванса
    }
    
    // Вывод результатов с форматированием и data-атрибутами для кликабельности
